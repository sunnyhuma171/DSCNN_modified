...
def load_data(revs,word_idx_map,sort_by_len=False,valid_portion=0.1): # default sort_by_len=True 
    train_set_x = []
    train_set_y = []
    valid_set_x = []
    valid_set_y = []
    ...
    print 'Reserve Validation:','Train',len(train_set_x), 'Test', len(test_set_x), 'Valid', len(valid_set_x)

    # sort by length
    def len_argsort(seq):
        return sorted(range(len(seq)), key = lambda x: len(seq[x]))

    if sort_by_len: # sort_by_len=False
    ...
    train = (train_set_x, train_set_y) # sort_by_len=False
    valid = (valid_set_x,valid_set_y) # sort_by_len=False
    test = (test_set_x, test_set_y) # sort_by_len=False
